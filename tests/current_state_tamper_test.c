...
/**
 * @brief Tests, that even tampering the current state manually will result in safe failure if in invalid state
 */
...
int main(void) {
    ...
    state_machine1.current_state = NULL;

    fsm_status = fsm_run(&state_machine1, event1);
    assert(fsm_status == FSM_STATUS_INVALID_STATE);
